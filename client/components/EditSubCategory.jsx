...
const EditSubCategory = ({ close, data, onSuccess }) => {
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [imgLoading, setImgLoading] = useState(false);
    const [selectedCategory, setSelectedCategory] = useState("");
    
    // Initialize state with the data passed in via props
        ...
        <section className='fixed inset-0 bg-neutral-800/70 flex items-center justify-center p-4 z-50'>
            <div className='w-full max-w-2xl bg-white p-4 rounded-lg shadow-lg'>
                <div className='flex justify-between items-center border-b pb-3 mb-4'>
                    <h1 className='text-xl font-semibold text-gray-800'>Edit Sub-Category</h1>
                ...
                <form onSubmit={handleSubmit} className='space-y-4'>
                    <div>
                        ...
                        <input type="text" id='edit-name' name='name' value={subCategoryData.name} onChange={handleChange} autoFocus className='w-full p-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500' />
                    </div>
                    ...
                    <div>
                        ...
                        <div className='flex items-center gap-4'>
                            ...
                            <label htmlFor="edit-uploadSubCategoryImage" className='cursor-pointer bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50'>
                                <span>Upload Image</span>
                            </label>
                            <input type="file" id='edit-uploadSubCategoryImage' hidden onChange={handleUploadSubCategoryImage} accept="image/*" />
                        </div>
                    </div>
