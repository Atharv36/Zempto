...
const EditAddressDetails = ({ close, data }) => {
  const { register, handleSubmit, reset } = useForm({
    defaultValues: {
        _id:data._id,
        userId:data.userId,
      addressline: data.addressline, // ✅ Fixed: use "addressline" instead of "address_line"
      city: data.city,
      state: data.state,
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const response = await Axios({
        ...
        data: {
            ...data,
              address_line: formData.addressline, // ✅ match backend name, // ✅ Mapping correctly to API
          city: formData.city,
    ...
    <section className='overflow-auto h-[100vh] bg-neutral-700/70 z-50 fixed top-0 bottom-0 left-0 right-0'>
      <div className='bg-white p-4 w-full max-w-lg mt-8 mx-auto rounded'>
        <div className='flex items-center justify-between'>
          <h2 className='font-semibold text-xl'>Edit Address</h2>
        ...
        <form className='grid gap-4 mt-4' onSubmit={handleSubmit(onSubmit)}>
          ...
          <button
            type='submit'
            className="rounded bg-green-500 text-white cursor-pointer w-full py-2 font-semibold hover:bg-green-600"
          >
            Submit
          </button>
        </form>
